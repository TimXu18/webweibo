    ...
    <div class="list">
        ...
        <?php if(is_array($value["data"])): ?><?php $i = 0;?><?php $__LIST__ = $value["data"]?><?php if( count($__LIST__)==0 ) : echo "" ; ?><?php else: ?><?php foreach($__LIST__ as $key=>$vo): ?><?php ++$i;?><?php $mod = ($i % 2 )?><tr overstyle='on' id="<?php echo ($vo['name']); ?>">
            ...
            <td>
                ...
                <?php }else{ ?>
                <?php $install_href = U('admin/Addons/startAddon',array('name'=>$vo['name'],'is_weixin'=>intval($_GET['is_weixin'])));
                if($vo['sqlfile']){
                ...
                <?php $href = U('admin/Addons/admin',array('pluginid'=>$vo['addonId'],'is_weixin'=>intval($_GET['is_weixin']))); ?>
