...
<div class="so_main">
  ...
  <div id="begin_backup" style="display:none;">
    ...
    <dl class="" id="backup_table" style="display:none">
        <dt>数据库表: </dt>
        <dd>
        <?php if(is_array($table)): ?><?php $i = 0;?><?php $__LIST__ = $table?><?php if( count($__LIST__)==0 ) : echo "" ; ?><?php else: ?><?php foreach($__LIST__ as $key=>$vo): ?><?php ++$i;?><?php $mod = ($i % 2 )?><input type="checkbox" name="backup_table[]" value="<?php echo ($vo["Name"]); ?>"><?php echo ($vo["Name"]); ?> ( <?php echo (admin_formatsize($vo['Data_length'])); ?> )<br/><?php endforeach; ?><?php endif; ?><?php else: echo "" ;?><?php endif; ?>
        </dd>
    </dl>
    <div class="page_btm">
  ...
  <?php if(empty($log)): ?>
  <tr><td colspan='100' align="center">没有备份</td></tr>
  <?php endif; ?>
