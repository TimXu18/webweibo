  	...
  	<tr>
		<!-- 左侧导航 -->
	  	<td width="150px" height='100%' valign="top" id="LeftMenu" rowspan="2">
	  		<div class="LeftMenu">
          	<a id="root_<?php echo ($iterator); ?>" class="title" href="<?php echo U('admin/Index/index');?>" onClick="switch_root_menu('<?php echo ($iterator); ?>');" hidefocus="true" style="outline:none;">管理中心</a>
			<?php $iterator = 1;
			  $home_url = ''; ?>
	  		<!-- 第一级菜单，即大频道 -->
			          ...
			          <ul id="tree_<?php echo ($iterator); ?>" class="submenu">
			            ...
			            <?php ++ $iterator; ?>
			          	...
			          	<?php foreach($menu_3 as $menu_3_name => $menu_3_url) { ?>
			            	...
			            	<li><a id="menu_<?php echo ($iterator); ?>" href="javascript:void(0)" onClick="switch_sub_menu('<?php echo ($iterator); ?>', '<?php echo ($menu_3_url); ?>');" class="submenuA" hidefocus="true" style="outline:none;"><?php echo (getshort($menu_3_name,10)); ?></a></li>
							<?php ++ $iterator; ?>
						<?php } ?>
			          </ul>
			        </li>
		...
		<td height="60px">
			<div class="header">
			    <div class="main-nav">
				    ...
				    <div class="nav-sub">
				    	<a style="padding:0px;" href="<?php echo U('public/Account/index');?>"><img src="<?php echo ($user["avatar_tiny"]); ?>"/></a><a href="<?php echo U('public/Index/index');?>"><?php echo L('SYSTEM_BACK_TO_FRONT');?></a><a href="javascript:void(0);" onClick="refresh();"><?php echo L('SYSTEM_REFRESH');?></a><a href="<?php echo U('admin/Public/logout');?>"><?php echo L('PUBLIC_LOGOUT');?></a><br/>
				    	<div id="TopTime"></div>
				    </div>
				</div>                   
			</div>
...
$(function() {
	...
	$.get('<?php echo U('admin/Upgrade/ajxjCheck');?>', function(data) {
